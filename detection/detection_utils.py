...
def fcos_get_deltas_from_locations(
    ...
    deltas = torch.zeros_like(gt_boxes[:, :4], device=gt_boxes.device)
    ...
    # Assigning -1 for background boxes
...
def fcos_apply_deltas_to_locations(
    ...
    # For cases when box if for background class
    backgroundIndices = deltas[:, 0] == -1
    output_boxes[backgroundIndices, 0] = locations[backgroundIndices, 0]    
    output_boxes[backgroundIndices, 2] = locations[backgroundIndices, 0]    
    output_boxes[backgroundIndices, 1] = locations[backgroundIndices, 1]   
    output_boxes[backgroundIndices, 3] = locations[backgroundIndices, 1]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    # Setting centerness -1 for background classes
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        # Iterating through range of heights and widths 
        h = [(num+0.5)*level_stride for num in range(feat_shape[2])]
        w = [(num+0.5)*level_stride for num in range(feat_shape[3])]
        ...
        location_coords[level_name] = torch.tensor(cords, device=device)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
